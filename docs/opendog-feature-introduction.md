...
| 指标 | 数据 |
|------|------|
| 源文件 | 226 个 Rust 源文件，约 57 000 行 |
| MCP 工具 | 27 个 AI 调用工具 + 2 个只读 Resource |
| CLI 命令 | 23 个顶层入口命令 |
| JSON 合约 | 33 个版本化 MCP payload contract |
| 测试 | 1 797 个单元/集成测试，0 clippy 警告 |
| 源码测试覆盖 | 113 / 226 个源文件包含 `#[cfg(test)]` 模块（50%） |
...
- **1797 个自动化测试**，覆盖纯函数逻辑、serde 序列化、SQLite 集成、分类引擎、证据匹配、配置解析、错误路径和边界条件。
- **0 clippy 警告**，所有编译器和 linter 检查均通过。
- **113 个源文件包含测试模块**，其中观测内核、存储层、MCP 辅助模块和 orphan 检测系统的测试密度最高。
- **版本化 MCP JSON contract**（33 个），确保 MCP 输出的 schema 稳定可消费。
