...
- Need overdesign review history and disposition chain: [reviews/README.md](./superpowers/reviews/README.md)
...
| Capability ownership and governance | [FUNCTION_TREE.md](../FUNCTION_TREE.md) | [.planning/REQUIREMENTS.md](../.planning/REQUIREMENTS.md) |
...
| Configuration policy, build diagnostics, and live reload | `FT-01.01.03` | What config is effective, what binary/schema is running, and did runtime pick up changes? | `get_global_config`, `get_build_info`, `get_project_config` | `opendog config ...`, `opendog self-update status` | [JSON Contracts](./json-contracts.md), [MCP Tool Reference](./mcp-tool-reference.md) |
...
| Comparative and time-window analytics | `FT-01.04.04` | What changed recently, which files are heating/cooling, and what historical volume survived retention cleanup? | `get_time_window_report`, `compare_snapshots`, `get_usage_trends`, `get_activity_rollups` | `opendog report window|compare|trend|rollup` | [AI Playbook](./ai-playbook.md), [MCP Tool Reference](./mcp-tool-reference.md) |
| Retained-evidence lifecycle | `FT-01.04.05` | Which OPENDOG-retained evidence should I prune, and is `VACUUM` worth it? | `get_activity_rollups` for post-cleanup volume checks; cleanup execution is CLI-only | `opendog cleanup-data` | [Storage Retention Runbook](./operations/storage-retention-runbook.md), [JSON Contracts](./json-contracts.md) |
...
- Default consumption path: start from `get_guidance`, `get_workspace_data_risk_overview`, or the matching CLI commands, then switch to shell only when OPENDOG explicitly points you to git, tests, or direct file inspection
...
| Need a broader “what next” recommendation | `get_guidance(detail=summary)` / `opendog agent-guidance` | Drill into the highest-priority project by `attention_score` or run the suggested report/verification step | You need semantic diff, tests, or direct file inspection to confirm the recommendation |
| Need to distinguish missing evidence from stale evidence first | `get_guidance(detail=summary)` / `opendog agent-guidance` | Read `layers.workspace_observation` and `project_overviews[*].observation` before choosing the next tool | You still need repo-native validation after OPENDOG identifies a stale snapshot or stale verification path |
| Need cross-project prioritization | `get_workspace_data_risk_overview` / `opendog workspace-data-risk` | `get_data_risk_candidates` or `get_guidance(detail=summary)` on the top project | Project ranking alone is not enough and you need repo-native validation |
...
| Need storage cleanup only | `opendog cleanup-data --dry-run` | Re-run without `--dry-run`, optionally with `--vacuum`; then inspect `get_activity_rollups` / `opendog report rollup` | You are unsure whether the target is OPENDOG-retained evidence or source data |
| Need historical volume after raw activity cleanup | `get_activity_rollups` / `opendog report rollup` | Use `get_usage_trends` for recent file-level detail | You need per-file or per-process raw history that retention cleanup already compacted |
