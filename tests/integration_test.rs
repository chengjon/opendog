...
fn test_registry_creates_schema() {
    ...
    let count: i64 = db
        .query_row("SELECT COUNT(*) FROM projects", rusqlite::params![], |row| row.get(0))
        .unwrap();
    assert_eq!(count, 1);
}
...
fn setup_manager() -> (TempDir, ProjectManager) {
    let dir = TempDir::new().unwrap();
...
fn setup_snapshot() -> (TempDir, Database) {
    let dir = TempDir::new().unwrap();
...
fn test_snapshot_filters_ignored_dirs() {
    ...
    let project_dir = dir.path().join("project");
    fs::create_dir_all(project_dir.join("src")).unwrap();
    fs::create_dir_all(project_dir.join("node_modules/pkg")).unwrap();
    fs::create_dir_all(project_dir.join(".git/objects")).unwrap();
    fs::create_dir_all(project_dir.join("dist")).unwrap();

    fs::write(project_dir.join("src/main.rs"), "fn main() {}").unwrap();
    fs::write(project_dir.join("node_modules/pkg/index.js"), "module.exports").unwrap();
...
fn test_snapshot_records_metadata() {
    ...
    let project_dir = dir.path().join("project");
    fs::create_dir_all(&project_dir).unwrap();
    fs::write(project_dir.join("code.rs"), "fn main() { println!(\"hi\"); }").unwrap();
    ...
    let entries = db.prepare_and_query(
...
fn test_snapshot_file_type_filter() {
    let (dir, db) = setup_snapshot();
    let project_dir = dir.path().join("project");
    fs::create_dir_all(project_dir.join("__pycache__")).unwrap();
    fs::create_dir_all(project_dir.join("src")).unwrap();

    fs::write(project_dir.join("main.py"), "print('hi')").unwrap();
    fs::write(project_dir.join("__pycache__/main.cpython-311.pyc"), "bytecode").unwrap();
    fs::write(project_dir.join("src/app.py"), "app").unwrap();

    let result = snapshot::take_snapshot(&db, &project_dir, &ProjectConfig::default()).unwrap();
    assert_eq!(result.total_files, 2); // main.py + src/app.py
}
