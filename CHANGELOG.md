...
- Agent guidance execution-strategy summary builders now live in `src/mcp/guidance_payload/execution_strategy.rs`, keeping the root guidance payload module focused on workspace envelope assembly.
- Decision-support risk profile models now live in `src/mcp/decision_support/profiles/model/risk.rs`, leaving the root profile model focused on action classification.
- Analysis payload regression tests now live in `src/mcp/payloads/analysis_payloads/tests.rs`, keeping the payload builder module focused on response assembly.
...
- Workspace toolchain aggregation now lives in `src/mcp/toolchain/workspace.rs`, leaving the root toolchain module focused on project-level profile detection.
- Review-focus and external-truth projection types now live in `src/mcp/guidance_types/repo_risk/projections.rs`, leaving the repo-risk type module focused on repo truth, risk findings, and strategy coupling.
...
- MCP guidance-scaffold regression tests now live in `src/mcp/guidance_scaffold/tests.rs`, keeping the scaffold helper focused on default guidance layer construction.
...
- Orphan module facade regression tests now live in `src/core/orphan/tests.rs`, keeping the root orphan module focused on submodule wiring and public re-exports.
- Orphan scan regression tests now live in `src/core/orphan/scan/tests.rs`, keeping the scan module focused on candidate discovery and external-report merging.
...
- Control-protocol request/response serialization regression tests now live in `src/control/protocol/tests.rs`, keeping the protocol module focused on daemon control message types and helpers.
...
- MCP governance-payload regression tests now live in `src/mcp/payloads/governance_payloads/tests.rs`, keeping the payload module focused on governance lane/node response shaping.
...
- Project-guidance lifecycle regression tests now live in `src/mcp/project_guidance/lifecycle/tests.rs`, keeping the lifecycle module focused on project setup and observation guidance payloads.
- Project-guidance unused-file regression tests now live in `src/mcp/project_guidance/stats_unused/unused/tests.rs`, keeping the unused-file module focused on classification and summary helpers.
...
- MCP project-recommendation scoring regression tests now live in `src/mcp/project_recommendation/scoring/tests.rs`, keeping the scoring module focused on signal weighting and action ranking logic.
...
- MCP external-truth constraint regression tests now live in `src/mcp/constraints/external_truth/tests.rs`, keeping the constraint helper focused on boundary, trigger, and summary construction.
...
- Decision-support entrypoint facade regression tests now live in `src/mcp/decision_support/entrypoints/tests.rs`, keeping the facade focused on entrypoint payload delegation.
- Decision-support execution-template facade regression tests now live in `src/mcp/decision_support/execution_templates/tests.rs`, keeping the facade focused on catalog and enrichment orchestration.
