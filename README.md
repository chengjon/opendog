...
>
> 当前状态：**全部能力已交付**。FUNCTION_TREE.md 中全部 22 个叶子节点（FT-01 观测内核 8 个、FT-02 服务交付 4 个、FT-03 AI 决策辅助 10 个）均已标记为 `shipped`。通过 MCP 20 个工具、2 个只读 Resource、22 个 CLI 命令、daemon + systemd 持久运行四种入口对外提供能力。
...
| AI 决策辅助 (FT-03) | 工作区观察、仓库风险、执行策略、验证证据、多项目优先级、清理/重构审查、工具链指导、约束/边界、MOCK / 硬编码数据审查 | MCP + CLI |
...
MCP surface 共 **20 个工具** + **2 个只读 Resource**，下面按能力簇分组。
