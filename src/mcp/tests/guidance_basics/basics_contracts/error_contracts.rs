...
#[test]
fn daemon_response_integrity_error_includes_remediation() {
    let Json(value) = error_json_for(
        MCP_GUIDANCE_V1,
        Some("demo"),
        &OpenDogError::DaemonResponseIntegrity(
            "daemon returned an incomplete JSON response".to_string(),
        ),
    );
    ...
    assert!(value["error"].as_str().unwrap().contains("integrity"));
    assert!(value["remediation"]["socket_path"]
        .as_str()
        .unwrap()
        .contains(".opendog/data/daemon.sock"));
    assert!(value["remediation"]["suggested_actions"]
        .as_array()
        .unwrap()
        .iter()
        .any(|action| action.as_str().unwrap().contains("equivalent CLI")));
}
