...
mod tests {
    ...
    fn cleanup_verification_scope_only_touches_verification_runs() {
        ...
        assert_eq!(result.deleted.verification_runs, 1);
        assert_eq!(result.deleted.file_sightings, 0, "verification scope must not touch activity");
        assert_eq!(result.deleted.file_events, 0);
    ...
    fn cleanup_dry_run_for_verification_scope_counts_but_preserves() {
        ...
        assert!(result.dry_run);
        assert_eq!(result.deleted.verification_runs, 1);
        assert_eq!(count(&db, "verification_runs"), 1, "dry run must not delete");
    }
    ...
    fn cleanup_no_matching_rows_produces_no_match_note() {
        ...
        assert_eq!(result.deleted.file_events, 0);
        assert!(result.notes.iter().any(|n| n.contains("no matching retained rows")));
    }
    ...
    fn cleanup_activity_scope_preserves_snapshots_and_verification() {
        ...
        assert_eq!(result.deleted.file_sightings, 1);
        assert_eq!(result.deleted.snapshot_runs, 0, "activity scope must not touch snapshots");
        assert_eq!(result.deleted.verification_runs, 0, "activity scope must not touch verification");
        assert_eq!(count(&db, "snapshot_runs"), 1);
        assert_eq!(count(&db, "verification_runs"), 1);
    }
    ...
    fn estimate_mode_scope_counts_only_above_threshold() {
        ...
        assert_eq!(result.deleted.snapshot_runs, 100, "prunable = total - keep_latest");
        assert_eq!(result.deleted.snapshot_history, 0, "history skipped in estimate mode");
        assert!(result.notes.iter().any(|n| n.contains("estimate-only mode")));
        // Nothing deleted
        assert_eq!(count(&db, "snapshot_runs"), 105);
    }
    ...
    fn estimate_mode_not_used_for_real_cleanup() {
        ...
        assert_eq!(result.estimate_mode, EstimateMode::Full, "real cleanup always uses full mode");
